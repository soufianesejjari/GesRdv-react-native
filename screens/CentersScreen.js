...
import { fetchCenters, selectCenters, selectLoading, selectError, centerSlice } from '../redux/reducers/CentresSlice';
import { useDispatch, useSelector } from 'react-redux';
import {selectCenter,selectLoadingCenter, fetchCenterInfo ,fetchCenterrelaod } from '../redux/reducers/CenterInfoSlice';
...
function CentersScreen({navigation}) {
  ...
  const handleCenterPress = async(item) => {
    try {
        ...
        navigation.navigate('CenterDetails', {center: center});
...
/*        if(!isStart){
  ...
  <Card.Content>
    <TouchableOpacity onPress={() => handleCenterPress(item)}>
      <Title style={{color:darkGreen}}>  {item.nom ? item.nom : "hh"}</Title>
...
const searchCentresByName = (centres, searchTerm) => {
  ...
  const filteredCentres = searchCentresByName(centers, searchTerm);
      ...
      return (
        ...
        <Animated.ScrollView
          contentContainerStyle={[styles.content,{backgroundColor:  backgroundC }]}
