...
const SearchSlotsScreen = ({ navigation }) => {
  ...
  const searchSlots = async () => {
    if (!centreId || !date) {
      Alert.alert(
        'Donnes Vides',
        'Vous devez obligatoirement sélectionner un centre et une date'
      )
      return
    }
  ...
  const handleBookSlot = async (slot) => {
    ...
    navigation.navigate('ConfirmationRenderVous', {
      rendezVous: normalizedRDV
    })
    ...
    <ScrollView style={styles.container}>
      ...
      {resultMode ? (
        ...
        <View style={styles.form}>
          <Text style={styles.inputLabel}>Select Centre </Text>
          <Picker
          ...
          >
            <Picker.Item label={centre || 'Select Centre'} value='' />
            {centres.map(
              (
                center,
          ...
          <Text style={styles.inputLabel}>Select Creneau (Optional)</Text>
