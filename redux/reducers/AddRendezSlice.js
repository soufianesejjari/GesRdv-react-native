...
export const {
  fetchaddRendezsStart,
  fetchaddRendezsSuccess,
  fetchaddRendezsFailure,
  fetchaddRendezsFinale
} = AddRendezSlice.actions

export const fetchaddRendez = (centre, date, creneau) => async (dispatch) => {
  dispatch(fetchaddRendezsStart())
  try {
    console.log(
      'Nous sommes dans la création du rendez-vous :',
      centre,
      date,
      creneau
    )

    const addRendez = await saveRendezVous({
      centreSante_Id: centre,
      crenea_Id: creneau,
      dateRendezVous: date
    })
    ...
    if (error.response) {
      ...
      console.error(
        'Erreur lors de la création du rendez-vousg:',
        error.message
      )
